...
export class Perfil implements OnInit {
  ...
  readonly roleConfigs: Record<RoleKey, ProfileRoleConfig> = {
    ADOTANTE: {
      label: "Adotante",
      descriptionFallback:
        "Complete seu perfil para facilitar conversas com ONGs e acompanhar pets favoritados.",
      managesPets: false,
    ...
    ONG: {
      label: "ONG",
      descriptionFallback:
        "Complete os dados da organização para transmitir confiança aos adotantes.",
      managesPets: true,
    ...
    PETSHOP: {
      label: "Petshop",
      descriptionFallback:
        "Complete os dados comerciais para divulgar serviços e receber contatos.",
      managesPets: true,
      businessProfile: true,
    ...
    ADMIN: {
      label: "Administrador",
      descriptionFallback:
        "Perfil administrativo com acesso ampliado para gestão da plataforma.",
      managesPets: true,
      businessProfile: false,
      businessNameLabel: "",
      businessDocumentLabel: "",
  ...
  readonly pets = signal<ProfilePet[]>([
    ...
    },
    {
      ...
      pelagem: "Curta",
      urlImagem:
        "https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?auto=format&fit=crop&q=80&w=500",
  ...
  readonly favoritePets = signal<ProfilePet[]>([
    {
      ...
      pelagem: "Longa",
      urlImagem:
        "https://images.unsplash.com/photo-1618826411640-d6df44dd3f7a?auto=format&fit=crop&q=80&w=500",
    ...
    },
    {
      ...
      pelagem: "Longa",
      urlImagem:
        "https://images.unsplash.com/photo-1517849845537-4d257902454a?auto=format&fit=crop&q=80&w=500",
      textoBadge: "Disponível",
      tipoBadge: "",
      favorito: true,
      mostrarCoracaoTop: true,
  ...
  readonly isBusinessProfile = computed(
    () => this.roleConfig().businessProfile,
  );
  ...
  readonly roleFields = computed<ProfileField[]>(() => {
    ...
    return [
      {
        label: this.roleConfig().businessNameLabel,
        icon: "fas fa-store",
        value: org.nomeFantasia || "",
      },
      {
        label: this.roleConfig().businessDocumentLabel,
        icon: "far fa-id-card",
        value: org.cnpj || "",
      },
      {
        label: this.roleConfig().businessEmailLabel,
        icon: "far fa-envelope",
        value: org.emailContato || "",
      },
      {
        label: this.roleConfig().businessPhoneLabel,
        icon: "fas fa-phone-alt",
        value: org.telefoneContato || "",
      },
      {
        label: this.roleConfig().businessAddressLabel,
  ...
  readonly profileDescription = computed(
    () =>
      this.user().organization?.descricao ||
      this.roleConfig().descriptionFallback,
  );
  ...
  readonly stats = computed(() => [
    {
      label: this.isBusinessProfile()
        ? "Pets associados"
        : "Buddies acompanhados",
      value: this.pets().length,
    },
    { label: "Favoritos", value: this.favoritePets().length },
    {
      label: "Mensagens não lidas",
      value: this.messages().filter((message) => message.unread).length,
    },
  ]);
  ...
  saveProfile(): void {
    ...
    const organizationPayload: BackendOrganization | null =
  ...
  removeFavorite(petName: string): void {
    this.favoritePets.update((pets) =>
      pets.filter((pet) => pet.nome !== petName),
    );
  }

  markAllMessagesAsRead(): void {
    this.messages.update((messages) =>
      messages.map((message) => ({ ...message, unread: false })),
    );
  }
  ...
  private applyRoleValidators(): void {
    const businessValidators: Partial<
      Record<keyof typeof this.profileForm.controls, ValidatorFn[]>
    > = {
    ...
    Object.entries(businessValidators).forEach(([field, validators]) => {
      const control =
        this.profileForm.controls[
          field as keyof typeof this.profileForm.controls
        ];
  ...
  private resolveDefaultPicture(role: RoleKey): string {
    const pictures: Record<RoleKey, string> = {
      ...
      ADMIN:
        "https://images.unsplash.com/photo-1556157382-97eda2d62296?auto=format&fit=crop&q=80&w=300",
    };
